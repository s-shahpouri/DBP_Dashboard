...
class OptunaInitializer():
    ...
    def __init__(self):
        ...
        self.exp_root_dir = self.directories_dict['root_dir']
        self.optuna_path_pickles = self.directories_dict['directories']['optuna_pickles_dir']
    ...
    def check_iteration_num(self):
        optuna_file_study_list = [x for x in os.listdir(self.optuna_path_pickles) if self.optuna_study_name in x]
        optuna_file_sampler_list = [x for x in os.listdir(self.optuna_path_pickles) if self.optuna_sampler_name in x]
        ...
        if len(optuna_file_study_list) > 0 and len(optuna_file_sampler_list) > 0:
            ...
            optuna_in_file_study = os.path.join(self.optuna_path_pickles, '{}_'.format(optuna_study_run_nr - 1) 
                                                + self.optuna_study_name)
            optuna_in_file_sampler = os.path.join(self.optuna_path_pickles, '{}_'.format(optuna_sampler_run_nr - 1) 
        ...
        return optuna_study, optuna_study_run_nr, optuna_sampler_run_nr
    ...
    def initializer(self):
        ...
        optuna_study, optuna_study_run_nr, optuna_sampler_run_nr = self.check_iteration_num()

        # Create study
        optuna_out_file_study = os.path.join(self.optuna_path_pickles, '{}_'.format(optuna_study_run_nr) + self.optuna_study_name)
        optuna_out_file_sampler = os.path.join(self.optuna_path_pickles, '{}_'.format(optuna_sampler_run_nr) + self.optuna_sampler_name)
