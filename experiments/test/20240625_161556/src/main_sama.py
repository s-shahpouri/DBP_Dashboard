...
#     choice = 'optuna' # "Enter 'optuna' to run Optuna trials or 'train' to run the training loop: ")
#     choice = choice.strip().lower()
#     main_obj = Main()
    
#     if choice == 'optuna':
#         main_obj.start_optuna(trial, param_dict)
        
#     elif choice == 'train':
#         main_obj.start_training()

#     else:
#         print("Invalid choice. Please enter 'optuna' or 'train'.")
"""
Foreword: The purpose of this pipeline is to make an infrastructure for some of the deep learning projects of the group. 
          I will complete it step by step and mainly based on different projects. 
Explanation: .....
...
class Main():
    ...
    def main(self, trial, param_dict, optuna_study, optuna_out_file_study, optuna_out_file_sampler):
        self.df_obj.set_seed(param_dict)
        trial_parameters_main = self.oo_obj.suggest_hyperparameters(trial, param_dict)
        logger = self.init_obj.initialize_logger(trial_parameters_main)
        train_dict, val_dict, test_dict = self.dl_obj.get_files(logger, trial_parameters_main)
        ...
        for fold in range(self.cv_dict['cv_folds']):
            # globals()['ijk'] += 1
            trial_parameters = trial_parameters_main.copy()
