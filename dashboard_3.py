...
import pandas as pd
import numpy as np
from vis import DataProcessor, display_coordination_info, load_css
...
back_dash = BackDash(main_root='/data/sama/Deep_Learning_Pipeline_Test/experiments_dose/test')
...
with st.sidebar:
    ...
    main_root = st.text_input("MAIN ROOT", "'/data/sama/Deep_Learning_Pipeline_Test/experiments_dose/test")


    st.divider()

    # Radio button for selecting the pickle file or folder
    selected_pickle = st.radio("Select Approach", list(pickle_files.keys()) + ["Select Folder"])

    if selected_pickle == "Select Folder":
